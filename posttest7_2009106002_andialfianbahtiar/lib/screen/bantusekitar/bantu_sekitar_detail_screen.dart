...
class BantuSekitarDetailScreen extends StatelessWidget {
  const BantuSekitarDetailScreen(
      {Key? key,
      required this.id,
        ...
        children: [
          ...
          Center(
            ...
            onPressed: () {
              ...
              bantusekitar.doc(id).update(
                {'bantuan': bantuan + 1},
              );
              CustomAlert(context, "Permintaan anda telah di simpan.");
            },
            child: Container(
              child: Center(child: Text("Bantu")),
              width: 350,
            ),
...
Future<dynamic> CustomAlert(BuildContext context, String pesan) {
  return showDialog(
    ...
    builder: (context) {
      return AlertDialog(
        title: const Text("Bantu Sekitar"),
        content: Text(pesan),
        actions: [
          TextButton(
              onPressed: () {
